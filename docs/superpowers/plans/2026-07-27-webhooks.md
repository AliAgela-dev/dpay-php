...
- What's NOT proven: that DPay's real infrastructure, given a real secret and a real reachable URL, actually delivers a request this code accepts. This needs the user to create a webhook endpoint at Dashboard -> Webhooks (sandbox mode), get a real secret, stand up a tunnel, and either use DPay's "Send test event" button (fires `webhook.test`, exercised structurally by this plan's tests but never received over real HTTP) or trigger a real sandbox payment.
